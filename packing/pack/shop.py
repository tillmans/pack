...
from pack.models import Shop,ShopFeedBack,WaiterOrder,Cook,WaiterServe,TransferMoney
...
def shopInfo(request):
    ...
    if _method == '0':
        ...
        _startTimeStamp = request.REQUEST.get('startTimeStamp')
        _endTimeStamp = request.REQUEST.get('endTimeStamp')
...
def shopWalletInfo(request):
    ...
    if _method == '0':
        ...
        shop.realName = _realName
        shop.cardNumber = _cardNumber
        shop.cardType = _cardType
    ...
    if _method == '2':
        response_data['realName'] = shop.realName
        response_data['cardNumber'] = shop.cardNumber
        response_data['cardType'] = shop.cardType
        response['code'] = 0
        response['data'] = response_data
        return HttpResponse(json.dumps(response),content_type="application/json")
    response['code'] = -1
...
def startService(request):
    ...
    if shop.table_set.count() == 0:
        ...
        return HttpResponse(json.dumps(response),content_type="application/json")
    if shop.waiterOrder_set.count() == 0:
        response['code'] = -1
    ...
    if shop.cook_set.count() == 0:
        ...
        return HttpResponse(json.dumps(response),content_type="application/json")
    if shop.waiterServe_set.count() == 0:
        response['code'] = -1
...
def shopFeedback(request):
    ...
    if _msg == None or _msg == '':
        ...
        return HttpResponse(json.dumps(response),content_type="application/json")
    shopFeedback = ShopFeedBack(shop = shop, msg = _msg)
    shopFeedback.save()
    response['code'] = 0
    return HttpResponse(json.dumps(response),content_type="application/json")
