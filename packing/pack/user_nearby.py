...
from pack.models import User,Shop,TableLock,Table,Order
...
from pack.tasks import checkTableStatusTask
...
def getShopsNearby(request):
    ...
    shopsQuery = shopsQuery.distance(current_point).filter(verifyStatus = '2').filter(isServiceOn = True).order_by(
        'distance').annotate(
    ...
    for shop in shops:
        ...
        _shop['tableNumber'] = shop.table_set().filter(status = '0').count()
...
def userLockTable(request):
    ...
    if str(table.status) == '1':
        tableLockQuery = table.tableLock_set.filter(isValid = True).all()
        if not tableLockQuery.exists():
            response['code'] = -1
            response['errorMsg'] = '锁定失败，请选择其他餐桌'
            return HttpResponse(json.dumps(response,ensure_ascii=False),content_type="application/json")
        tableLock = tableLockQuery[0]
        if tableLock.user == user:
            tableLock.date = datetime.datetime.now()
 #           checkTableStatusTask.delay(lockTimeStamp,_userId,_tableId)
            response['code'] = 0
            response['data'] = str(tableLock.id)
            return HttpResponse(json.dumps(response),content_type="application/json")
        else:
            response['code'] = -1
        ...
        table.status = '1'
        tableLock  = TableLock(table = table,user = user)
        tableLock.save()
#        checkTableStatusTask.delay(lockTimeStamp,_userId,_tableId)
        response['code'] = 0
        response['data'] = str(tableLock.id)
